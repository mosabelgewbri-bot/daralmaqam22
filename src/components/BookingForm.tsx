...
export default function BookingForm({ user }: { user: User }) {
  ...
  const calculateTotals = () => {
    ...
    const baseTotalLYD = ticketsLYD + packageLYD;
    const baseTotalUSD = ticketsUSD + packageUSD;
    
    // Calculate Final Totals specifically per currency
    const finalLYD = Math.max(0, baseTotalLYD - (discountLYD || 0));
    const finalUSD = Math.max(0, baseTotalUSD - (discountUSD || 0));
    ...
    return {
      ...
      baseTotalLYD,
      baseTotalUSD,
      baseCombinedTotalLYD: baseTotalLYD + (baseTotalUSD * (exchangeRate || 0)),
      discountLYD,
      discountUSD
    };
  };
        ...
        <div style="width: 450px; background: #1a1a1a; color: #fff; padding: 30px; border-radius: 20px; border: 4px solid #d4af37;">
          <div style="display: flex; justify-content: space-between; margin-bottom: 15px; font-size: 18px;">
            ...
            <span>
              ...
              ${(savedBooking.totals.baseTotalUSD || savedBooking.totals.totalUSD) > 0 ? ' + ' + (savedBooking.totals.baseTotalUSD || savedBooking.totals.totalUSD).toLocaleString() + ' دولار' : ''}
            </span>
          </div>
      ...
      >
        <div className="glass-card w-full max-w-4xl p-8 space-y-8 border-gold/30">
          ...
          <div className="flex justify-between gap-12 text-sm text-white/60">
            <span>الإجمالي (قبل التخفيض)</span>
            <span>
              {(savedBooking.totals.baseTotalLYD || savedBooking.totals.totalLYD) > 0 && `${(savedBooking.totals.baseTotalLYD || savedBooking.totals.totalLYD).toLocaleString()} د.ل`}
              {(savedBooking.totals.baseTotalLYD || savedBooking.totals.totalLYD) > 0 && (savedBooking.totals.baseTotalUSD || savedBooking.totals.totalUSD) > 0 && ' + '}
              {(savedBooking.totals.baseTotalUSD || savedBooking.totals.totalUSD) > 0 && `${(savedBooking.totals.baseTotalUSD || savedBooking.totals.totalUSD).toLocaleString()} دولار`}
            </span>
          </div>
          {savedBooking.discountLYD > 0 && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="glass-card p-6 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <select 
                ref={(el) => { if (el) fieldRefs.current['selectedTripId'] = el; }}
        ...
        <div className="glass-card p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="space-y-2 text-right">
              ...
              <input 
                ref={(el) => { if (el) fieldRefs.current['regId'] = el; }}
                type="text" 
                ...
                {pilgrims.map((p, idx) => (
                  <tr key={idx} className="hover:bg-white/5 transition-colors">
                    <td className="px-6 py-4">
                      <input 
                        ref={(el) => { if (el) fieldRefs.current[`pilgrimName-${idx}`] = el; }}
                        type="text" 
                        className={clsx(
                    ...
                    <td className="px-6 py-4">
                      <div className="space-y-2">
                        <div className="relative">
                          <input 
                            ref={(el) => { if (el) fieldRefs.current[`pilgrimPassportNo-${idx}`] = el; }}
                            type="text" 
                            className={clsx(
                        ...
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-2">
                            <input 
                              ref={(el) => { if (el) fieldRefs.current[`pilgrimExpiryDate-${idx}`] = el; }}
                              type="date" 
                              className={clsx(
            ...
            <div className="space-y-4">
              {['Double', 'Triple', 'Quad', 'Quint', 'VisaOnly']
                ...
                <div key={type} className="flex items-center gap-4">
                  ...
                  <input 
                    ref={(el) => { if (el) fieldRefs.current[`roomPrice-${type}`] = el; }}
                    type="number" 
